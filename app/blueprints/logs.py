...
def analyser_logs(texte):
    ...
    for ip, total in ip_requetes.most_common(10):
        ...
        if a['critique'] > 0:      statut = 'CRITIQUE'
        elif a['elevated'] > 2:    statut = 'ÉLEVÉ'
        elif sum(a.values()) > 0:  statut = 'SUSPECT'
        else:                      statut = 'NORMAL'
        resultats.append({'ip':ip,'total':total,'critique':a['critique'],'elevated':a['elevated'],'statut':statut})
    return {'resultats':resultats,'total_lignes':len(lignes),'total_ips':len(ip_requetes),'total_requetes':len(toutes_ips)}
    ...
    if request.method == 'POST':
        ...
        if not fichier or fichier.filename == '':
            ...
            try:
                ...
                analyse = analyser_logs(texte) if texte.strip() else None
                if not analyse:
                    erreur = "Fichier vide."
            except Exception as e:
                erreur = f"Erreur : {e}"
    return render_template('logs.html', active='logs', analyse=analyse, erreur=erreur)
